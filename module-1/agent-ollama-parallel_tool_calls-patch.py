...
# %%
%%capture --no-stderr
%pip install --quiet -U langchain_openai langchain_core langgraph langgraph-prebuilt langchain_ollama
...
class PatchedChatOllama(ChatOllama):
    def _chat_params(self, *args, **kwargs):
        params = super()._chat_params(*args, **kwargs)
        params.pop("parallel_tool_calls", None)  # 🔥 remove it
        return params
...
llm = PatchedChatOllama(model="gpt-oss:20b")
#llm = ChatOllama(model="gpt-oss:20b",temperature="0").bind_tools(tools, parallel_tool_calls=False) #not working
